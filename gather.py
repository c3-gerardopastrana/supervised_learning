...
class _Reduce_Scatter(Function):
    ...
    def forward(ctx, op, group, tensor, *input_tensor_list):
        logger.debug(f"Rank {dist.get_rank()}: Starting reduce_scatter forward")
        ctx.group = group
        ...
        logger.debug(f"Rank {dist.get_rank()}: Reduced tensor shape: {tensor.shape}")
        logger.debug(f"Rank {dist.get_rank()}: Input tensor list shapes: {[t.shape for t in input_tensor_list]}")

        dist.reduce_scatter(tensor, list(input_tensor_list), op=op, group=group)
        logger.debug(f"Rank {dist.get_rank()}: Completed reduce_scatter")

        return tensor

    @staticmethod
    def backward(ctx, grad_output):
        logger.debug(f"Rank {dist.get_rank()}: Starting reduce_scatter backward")
        # In reduce_scatter, we don't need gradients for op, group, tensor, or input_tensor_list
...
class _AllGather(Function):
    ...
    def forward(ctx, group, tensor):
        logger.debug(f"Rank {dist.get_rank()}: Starting all_gather forward")
        ...
        logger.debug(f"Rank {dist.get_rank()}: Preparing to all_gather tensor with shape {tensor.shape}")
        dist.all_gather(out_tensor_list, tensor, group=group)

        logger.debug(f"Rank {dist.get_rank()}: Completed all_gather, output tensor shapes: {[t.shape for t in out_tensor_list]}")
        return tuple(out_tensor_list)

    @staticmethod
    def backward(ctx, *grad_outputs):
        logger.debug(f"Rank {dist.get_rank()}: Starting all_gather backward")

        if dist.get_backend(group=ctx.group) is dist.Backend.NCCL:
            rank = dist.get_rank()
            gx = torch.empty_like(grad_outputs[rank])
            logger.debug(f"Rank {dist.get_rank()}: NCCL backend detected, calling _Reduce_Scatter for gradients")
            _Reduce_Scatter.apply(ReduceOp.SUM, ctx.group, gx, *grad_outputs)
        else:
            # Fallback to AlltoAll if ReduceScatter is not available
            tensor_list = [torch.empty_like(tensor) for tensor in grad_outputs]
            logger.debug(f"Rank {dist.get_rank()}: Calling _AlltoAll for gradients (sum them later)")
            gxs = _AlltoAll.apply(ctx.group, tensor_list, *grad_outputs)
            gx = torch.sum(torch.stack(gxs), dim=0)
        
        logger.debug(f"Rank {dist.get_rank()}: Completed all_gather backward, gradient shape: {gx.shape}")
        return (None, gx)
