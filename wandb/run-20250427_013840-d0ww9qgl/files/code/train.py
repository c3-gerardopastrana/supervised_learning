...
def get_scaled_lr_sqrt(k: int, k0: int = 7, base_lr: float = 1e-3) -> float:
    """
    Scales the learning rate with sqrt of batch size increase, batch size = 2^k.

    Args:
        k (int): log2 of the new batch size
        k0 (int): log2 of the original batch size corresponding to base_lr
        base_lr (float): base learning rate at batch size 2^k0
...
def train_worker(rank, world_size, config):
    ...
    solver = Solver(
        ...
        lr =  get_scaled_lr_sqrt(torch.log2(config['batch_size'])) 
    )
