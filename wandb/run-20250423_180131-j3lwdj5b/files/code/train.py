...
class ResNet(nn.Module):
    def __init__(self, block, num_blocks, num_classes=10, lda_args=None):
        ...
        self.projector = nn.Linear(512 * block.expansion, 9)
    ...
    #     )
        ...
        if self.lda_args:
            ...
            return hasComplexEVal, fea, out, sigma_w_inv_b, out_ce
        else:
            out = self.linear(fea)
            return out
        ...
        for batch_idx, (inputs, targets) in enumerate(dataloader):
            ...
            if self.use_lda:
                ...
                if not hasComplexEVal:
                    ...
                    def get_ce_weight(epoch, warmup_epochs=7, decay_epochs=40):
                        """
                    ...
                    def get_loss_weights(epoch, total_epochs=30):
                        # Initial phase: Strong CE guidance
                        if epoch < total_epochs * 0.3:
                            ce_weight = 1.0
                            lda_weight = max(0.0, epoch / (total_epochs * 0.3))
                        # Middle phase: Equal contribution
                        elif epoch < total_epochs * 0.7:
                            transition = (epoch - total_epochs * 0.3) / (total_epochs * 0.4)
                            ce_weight = 1.0 - 0.5 * transition
                            lda_weight = 0.5 + 0.5 * transition
                        # Final phase: LDA refinement
                        else:
                            transition = (epoch - total_epochs * 0.7) / (total_epochs * 0.3)
                            ce_weight = 0.5 * (1.0 - transition)
                            lda_weight = 1.0
                            
                        return ce_weight, lda_weight
                        
                    ce_weight, lda_weight = get_loss_weights(epoch)
                    loss = lda_weight * self.criterion(sigma_w_inv_b) + ce_weight * F.cross_entropy(ce, targets)
