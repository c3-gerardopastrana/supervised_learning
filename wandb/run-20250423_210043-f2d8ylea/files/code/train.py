        ...
        for batch_idx, (inputs, targets) in enumerate(dataloader):
            ...
            if self.use_lda:
                ...
                if not hasComplexEVal:
                    ...
                    def get_loss_weights(epoch, warm_up=10, total_epochs=20):
                        # Initial phase: Strong CE guidance
                        if epoch < warm_up:
                            ce_weight = 1.0
                            lda_weight = 0
                        else:
                            max_epoch = total_epochs - warm_up
                            ce_weight = max(0.0, 1.0 - (epoch - warm_up)/ max_epoch)
                            lda_weight = 1 - ce_weight
                         
                        return ce_weight, lda_weight
                        #return max(ce_weight,0.1), min(lda_weight,.9)
